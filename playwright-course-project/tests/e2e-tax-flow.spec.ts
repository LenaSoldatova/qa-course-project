...
import { IncomingsPage } from '@pages/incomingsPage';
...
test('E2E test: add Incomings, add Expenses, post tax, check reporting', async ({ page }) => {
  ...
  await test.step('Calculate totals for income and expenses', async () => {
    const incomingsPage = new IncomingsPage(page);
    incomeTotal = await incomingsPage.getIncomeTotal();
    const expensesPage = new ExpensesPage(page);
    expenseTotal = await expensesPage.getExpenseTotal();
  });
